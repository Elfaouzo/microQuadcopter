...
void LSM6::enableDefault_Quad(void)
{
  if (_device == device_DS33)
  {
    ...
    // 0x80 = 0b10000000
    // ODR = 1000 (1.66 kHz (high performance)); FS_XL = 00 (+/-2 g full scale)
    //Objective: Range is set to +/- 2g
    //writeReg(CTRL1_XL, 0x80);
    
    // 0x8C = 0b10001100
    // ODR = 1000 (1.66 kHz (high performance)); FS_XL = 11 (+/-8 g full scale)
    //Objective: Range is set to +/- 8g
    writeReg(CTRL1_XL, 0x8C);
    
    
    // 0x80 = 0b10000000                                                            //Enables LPF on ACC.
    writeReg(CTRL8_XL, 0x80);

    /////// GYRO /////////
    // 0x80 = 0b10000000
    // ODR = 1000 (1.66 kHz (high performance)); FS_G = 00 (250 dps)
    //Objective: 250deg/s, sampling time is 1 KHz, sensitivity factor 131. Cut off freq = 5Hz?
    //writeReg(CTRL2_G, 0x80);
    
    // 0x84 = 0b10000100
    // ODR = 1000 (1.66 kHz (high performance)); FS_G = 01 (500 dps)
    //Objective: 500deg/s, sampling time is 1 KHz, sensitivity factor 65.5
